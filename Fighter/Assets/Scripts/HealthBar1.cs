...
public class HealthBar1 : MonoBehaviour
{
    ...
    void Update()
    {
        if (targetEntity.GetComponent<EnemyController>() != null)
            gameObject.GetComponent<Slider>().value = targetEntity.GetComponent<EnemyController>().health/200;
        ...
        {
            // change width of rectTransform
            gameObject.GetComponent<Slider>().value = targetEntity.GetComponent<PlayerController>().health/200;
